...
public final class WildexMobPreviewRenderer {
    ...
    public void render(
        ...
        float scaleF = (maxSizePx / dim) * zoom;
        int scale = Math.max(1, Math.round(scaleF));
        ...
        if (hiddenUndiscovered) RenderSystem.setShaderColor(0f, 0f, 0f, 1f);
        try {
            int drawY = cy + Math.round(mob.getBbHeight() * scaleF * 0.5f);
            ...
            if (fish) {
                base.rotateY((float) Math.toRadians(yaw));
                base.rotateX((float) Math.toRadians(FISH_MODEL_PITCH_DEG));
                base.rotateY((float) Math.toRadians(FISH_MODEL_SIDE_YAW_DEG));
            }
